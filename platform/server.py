...
SENTINEL_DIR = os.environ.get("SENTINEL_DATA_DIR", "/app/data/sentinel")
os.makedirs(SENTINEL_DIR, exist_ok=True)
UPLOAD_CHUNK = 64 * 1024 * 1024  # 64 MB
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/sentinel/files":
            files = []
            for fn in sorted(os.listdir(SENTINEL_DIR), reverse=True):
                if fn.endswith(".zip") and fn.startswith("S1"):
                    files.append(fn)
            self._json(files)
            return
    ...
    def do_POST(self):
        ...
        if path == "/api/upload":
            ...
            if not filename or not filename.endswith(".zip"):
                self._json({"error": "X-Filename header manquant ou invalide"}, 400)
                return
            # Sous-dossier par slot (before/after) pour éviter collision entre les deux widgets
            slot = self.headers.get("X-Slot", "")
            slot = os.path.basename(slot) if slot else ""
            upload_dir = os.path.join(SENTINEL_DIR, slot) if slot else SENTINEL_DIR
            ...
            try:
                with open(dest, "wb") as f:
                    while written < total:
                        to_read = min(UPLOAD_CHUNK, total - written)
                        chunk = self.rfile.read(to_read)
                        if not chunk:
                            break
                        f.write(chunk)
                        written += len(chunk)
                self._json({"ok": True, "file": filename, "size": written})
